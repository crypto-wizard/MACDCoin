...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 1;	// stake age of full weight:  1 days
unsigned int nStakeTargetSpacing = 60;		// 60 sec block spacing

int64 nChainStartTime = 1403666666;
...
const string strMessageMagic = "MACDCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    else if(nHeight > 7000 && nHeight <= 8500)
        nSubsidy = 1250 * COIN;
    
    else if(nHeight > 8500 && nHeight <= 10000)
        nSubsidy = 1500 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    nRewardCoinYear = MAX_MACD_PROOF_OF_STAKE;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in MACDCoin
    bool fStrictPayToScriptHash = true; // Always active in MACDCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MACDCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "29 May 2014 - Asgard - Norse Scam Developers on the Loose";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0xe4bc7c43b321edee7ea5239da560ad28bc4cccab074fd89f7b51510bc5d39312"));
        ...
        if (fProofOfStake)
        {
            ...
            if (pblock->IsProofOfStake())
            {
                if (!pblock->SignBlock(*pwalletMain))
                    continue;
                printf("Always trust the MACD! A proof-of-stake block has been found! %s\n", pblock->GetHash().ToString().c_str());
